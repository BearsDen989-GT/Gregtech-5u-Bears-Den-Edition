...
public class ProcessingCrushedPurified implements gregtech.api.interfaces.IOreRecipeRegistrator {
    ...
    public void registerOre(OrePrefixes aPrefix, MaterialsOld aMaterial, String aOreDictName, String aModName, ItemStack aStack) {
        ...
        if (aMaterial.contains(SubTag.WASHING_BLUEV))
            RECIPE_ADDER_INSTANCE.addChemicalBathRecipe(GT_Utility.copyAmount(1L, aStack), MaterialsOld.SulfuricAcid.getFluid(3000L), new FluidStack(ItemList.sBlueVitriol,3000), GT_OreDictUnificator.get(OrePrefixes.crushedCentrifuged, aMaterial, 1), GT_OreDictUnificator.get(OrePrefixes.dustTiny, aMaterial, 3), NULL_ITEM_STACK,  new int[]{10000, 5000}, 800, 2);
        if (aMaterial.contains(SubTag.WASHING_GREENV))
            RECIPE_ADDER_INSTANCE.addChemicalBathRecipe(GT_Utility.copyAmount(1L, aStack), MaterialsOld.SulfuricAcid.getFluid(3000L), new FluidStack(ItemList.sGreenVitriol,3000), GT_OreDictUnificator.get(OrePrefixes.crushedCentrifuged, aMaterial, 1), GT_OreDictUnificator.get(OrePrefixes.dustTiny, aMaterial, 3), NULL_ITEM_STACK,  new int[]{10000, 5000}, 800, 2);
        if (aMaterial.contains(SubTag.WASHING_NICKELS))
            RECIPE_ADDER_INSTANCE.addChemicalBathRecipe(GT_Utility.copyAmount(1L, aStack), MaterialsOld.SulfuricAcid.getFluid(3000L), new FluidStack(ItemList.sNickelSulfate,3000), GT_OreDictUnificator.get(OrePrefixes.crushedCentrifuged, aMaterial, 1), GT_OreDictUnificator.get(OrePrefixes.dustTiny, aMaterial, 3), NULL_ITEM_STACK,  new int[]{10000, 5000}, 800, 2);
    }
}
