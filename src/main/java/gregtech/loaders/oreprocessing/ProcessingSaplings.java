...
public class ProcessingSaplings implements gregtech.api.interfaces.IOreRecipeRegistrator {
    ...
    public void registerOre(OrePrefixes aPrefix, MaterialsOld aMaterial, String aOreDictName, String aModName, ItemStack aStack) {
        GT_ModHandler.addPulverisationRecipe(GT_Utility.copyAmount(1L, aStack), GT_OreDictUnificator.get(OrePrefixes.dustSmall, MaterialsOld.Wood, 2L), null, 0, false);
        GT_ModHandler.addCompressionRecipe(GT_Utility.copyAmount(4L, aStack), ItemList.IC2_Plantball.get(1L));
        RECIPE_ADDER_INSTANCE.addLatheRecipe(GT_Utility.copyAmount(1L, aStack), GT_OreDictUnificator.get(OrePrefixes.stick, MaterialsOld.Wood, 1L), GT_OreDictUnificator.get(OrePrefixes.dustTiny, MaterialsOld.Wood, 1L), 16, 8);
    }
}
