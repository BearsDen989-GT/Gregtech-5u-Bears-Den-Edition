...
public class EX_MachineRecipeLoader {
	...
	private final static void run(){
		...
		GT_Values.RA.addBlastRecipe(GT_OreDictUnificator.get(OrePrefixes.ingot, Materials.Titanium, 1L), GT_OreDictUnificator.get(OrePrefixes.ingot, Materials.Tantaloy60, 1L), GT_Values.NF, GT_Values.NF, GT_OreDictUnificator.get(OrePrefixes.ingotHot, Materials.Tantaloy61, 1L), GT_OreDictUnificator.get(OrePrefixes.dustSmall, Materials.DarkAsh, 1L), (int) Math.max(Materials.Tantaloy61.getMass() / 80L, 1L) * Materials.Tantaloy61.mBlastFurnaceTemp, 480, Materials.Tantaloy61.mBlastFurnaceTemp);
		GT_Values.RA.addBlastRecipe(GT_OreDictUnificator.get(OrePrefixes.ingot, Materials.Titanium, 1L), GT_OreDictUnificator.get(OrePrefixes.ingot, Materials.Uranium, 9L), GT_Values.NF, GT_Values.NF, GT_OreDictUnificator.get(OrePrefixes.ingotHot, Materials.Staballoy, 1L), GT_OreDictUnificator.get(OrePrefixes.dustSmall, Materials.Carbon, 1L), (int) Math.max(Materials.Staballoy.getMass() / 80L, 1L) * Materials.Staballoy.mBlastFurnaceTemp, 1000, Materials.Staballoy.mBlastFurnaceTemp);
